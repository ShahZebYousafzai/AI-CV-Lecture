...
> The repository ships `Makefile.txt`. Rename it to `Makefile` to use the `make`
> shortcuts, or ignore it: every command is spelled out below.
...
├── docker-compose.yml
├── requirements*.txt
└── Makefile.txt              rename to Makefile to use `make`
```
